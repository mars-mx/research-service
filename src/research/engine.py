...
# Depth-tier presets: (report_type, depth, breadth)
DEPTH_TIERS: dict[str, tuple[str, int, int]] = {
    "quick": ("research_report", 1, 2),
    "standard": ("research_report", 2, 4),
    "deep": ("detailed_report", 3, 6),
}
...
class ResearchEngine:
    ...
    async def run(
        ...
        await emit("started", {"task_id": task_id})

        # --- Stage 1: Plan sub-queries ---
        await emit("status", {"step": "planning", "message": "Generating research questions..."})
        ...
        await emit("status", {"step": "writing", "message": "Generating final report..."})
        combined_context = "\n\n---\n\n".join(all_context)

        # Trim context to ~25k words to stay within token limits
        words = combined_context.split()
        if len(words) > 25000:
            combined_context = " ".join(words[:25000])
        ...
        total_output += write_result.usage.output_tokens or 0
        ...
        await emit("result", {"task_id": task_id, "report": report, "sources": [s.model_dump() for s in unique_sources]})
        await emit("done", {})
    ...
    async def _research_level(
        ...
        total_output += plan_result.usage.output_tokens or 0
        total_requests += plan_result.usage.requests or 0

        # Search all sub-queries in parallel
        await emit("status", {"step": "researching", "message": f"Searching {len(sub_queries)} queries..."})
        search_tasks = [
        ...
        for results in search_results_list:
            for r in results:
                ...
                await emit("finding", {"source": r.url, "summary": r.snippet[:200]})

        urls_to_scrape = list({r.url for r in all_search_results})

        # Scrape in parallel (Firecrawl handles batching internally)
        await emit("status", {"step": "researching", "message": f"Scraping {len(urls_to_scrape)} pages..."})
        ...
        if passages:
            await emit("status", {"step": "researching", "message": "Compressing context..."})
            passages = await compress_context(
                query=query,
                passages=passages,
